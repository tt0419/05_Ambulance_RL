...
class EMSEnvironment:
    ...
    def _calculate_ambulance_completion_time(self, ambulance_id: int, call: Dict, response_time: float) -> float:
        ...
        # 4. 病院選択と搬送時間（validation_simulation互換）
        hospital_h3 = self.select_hospital(call['h3_index'], severity)
        if hospital_h3 is None:
            # 病院が見つからない場合のフォールバック（現場と同じ場所として扱う）
            hospital_h3 = call['h3_index']
        transport_time = self._calculate_travel_time_for_phase(call['h3_index'], hospital_h3, phase='transport') / 60.0
        ...
        # 8. 帰署時間（validation_simulation互換）
        amb_state = self.ambulance_states[ambulance_id]
        return_time = self._calculate_travel_time_for_phase(hospital_h3, amb_state['station_h3'], phase='return') / 60.0
